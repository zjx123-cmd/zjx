...
Date.ext = {}, Date.ext.util = {}, Date.ext.util.xPad = function (a, b, c) {
    ...
    return a.toString()
}, Date.prototype.locale = "en-GB", document.getElementsByTagName("html") && document.getElementsByTagName("html")[0].lang && (Date.prototype.locale = document.getElementsByTagName("html")[0].lang), Date.ext.locales = {}, Date.ext.locales.en = {a: ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"], A: ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"], b: ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"], B: ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"], c: "%a %d %b %Y %T %Z", p: ["AM", "PM"], P: ["am", "pm"], x: "%d/%m/%y", X: "%T"}, Date.ext.locales["en-US"] = Date.ext.locales.en, Date.ext.locales["en-US"].c = "%a %d %b %Y %r %Z", Date.ext.locales["en-US"].x = "%D", Date.ext.locales["en-US"].X = "%r", Date.ext.locales["en-GB"] = Date.ext.locales.en, Date.ext.locales["en-AU"] = Date.ext.locales["en-GB"], Date.ext.formats = {a: function (a) {
    ...
    var b = a.getFullYear(), c = parseInt(Date.ext.formats.V(a), 10), d = parseInt(Date.ext.formats.W(a), 10);
    return d > c ? b++ : 0 === d && c >= 52 && b--, b
}, H: ["getHours", "0"], I: function (a) {
    var b = a.getHours() % 12;
    ...
    var b = parseInt(Date.ext.formats.j(a), 10), c = 6 - a.getDay(), d = parseInt((b + c) / 7, 10);
    return Date.ext.util.xPad(d, 0)
}, V: function (a) {
    var b = parseInt(Date.ext.formats.W(a), 10), c = new Date("" + a.getFullYear() + "/1/1").getDay(), d = b + (c > 4 || 1 >= c ? 0 : 1);
...
}, w: "getDay", W: function (a) {
    var b = parseInt(Date.ext.formats.j(a), 10), c = 7 - Date.ext.formats.u(a), d = parseInt((b + c) / 7, 10);
    return Date.ext.util.xPad(d, 0, 10)
}, y: function (a) {
    return Date.ext.util.xPad(a.getFullYear() % 100, 0)
}, Y: "getFullYear", z: function (a) {
    var b = a.getTimezoneOffset(), c = Date.ext.util.xPad(parseInt(Math.abs(b / 60), 10), 0), d = Date.ext.util.xPad(b % 60, 0);
    return(b > 0 ? "-" : "+") + c + d
}, Z: function (a) {
    return a.toString().replace(/^.*\(([^)]+)\)$/, "$1")
}, "%": function () {
    return"%"
}}, Date.ext.aggregates = {c: "locale", D: "%m/%d/%y", h: "%b", n: "\n", r: "%I:%M:%S %p", R: "%H:%M", t: "	", T: "%H:%M:%S", x: "locale", X: "locale"}, Date.ext.aggregates.z = Date.ext.formats.z(new Date), Date.ext.aggregates.Z = Date.ext.formats.Z(new Date), Date.ext.unsupported = {}, Date.prototype.strftime = function (a) {
    ...
    for (var b = 0; b < a.length; ++b) {
        ...
        if (f) {
            var g = d.exec(f[1]), h = g[1], i = g[2], j = g[3] || 0;
    ...
    for (var c, d, e, f = /function\s+([^(]*?)\s*\(([^)]*)\)/, g = /['"]?([0-9A-Za-z_]+)['"]?\s*[:=]\s*function\b/, h = /['"]?([0-9A-Za-z_]+)['"]?\s*[:=]\s*(?:eval|new Function)\b/, i = "", j = Math.min(b, 20), k = 0; j > k; ++k)if (c = a[b - k - 1], e = c.indexOf("//"), e >= 0 && (c = c.substr(0, e)), c) {
...
var DygraphOptions = function () {
    ...
    return a.AXIS_STRING_MAPPINGS_ = {y: 0, Y: 0, y1: 0, Y1: 0, y2: 1, Y2: 1}, a.axisToIndex_ = function (b) {
        if ("string" == typeof b) {
        ...
        if (b) {
            ...
            if (c) {
                for (var d = 0, e = 0; e < this.labels_.length; e++) {
                    ...
                    "object" == typeof i && (h = ++d, this.yAxes_[h] = {series: [f], options: i}), i || this.yAxes_[0].series.push(f), this.series_[f] = {idx: e, yAxis: h, options: g}
                }
                ...
                this.series_[f] = {idx: e, yAxis: h, options: g}, this.yAxes_[h] ? this.yAxes_[h].series.push(f) : this.yAxes_[h] = {series: [f], options: {}}
            }
...
DygraphLayout.prototype.attr_ = function (a) {
    ...
    var b = {chart_div: this.dygraph_.graphDiv, reserveSpaceLeft: function (b) {
    ...
    for (var a = this.attr_("connectSeparatedPoints"), b = this.attr_("stackedGraph"), c = this.attr_("errorBars") || this.attr_("customBars"), d = 0; d < this.points.length; d++)for (var e = this.points[d], f = this.setNames[d], g = this.dygraph_.axisPropertiesForSeries(f), h = this.dygraph_.attributes_.getForSeries("logscale", f), i = 0; i < e.length; i++) {
        var j = e[i];
    ...
    for (this.yticks = [], a = 0; a < this.yAxes_.length; a++)for (var e = this.yAxes_[a], f = 0; f < e.ticks.length; f++)b = e.ticks[f], c = b.label, d = this.dygraph_.toPercentYCoord(b.v, a), d >= 0 && 1 >= d && this.yticks.push([a, d, c])
...
var DygraphCanvasRenderer = function (a, b, c, d) {
    ...
    if (this.area = d.getPlotArea(), this.container.style.position = "relative", this.container.style.width = this.width + "px", this.dygraph_.isUsingExcanvas_)this._createIEClipArea(); else if (!Dygraph.isAndroid()) {
...
DygraphCanvasRenderer.prototype.attr_ = function (a, b) {
    ...
    var h = this.area;
    a({x: 0, y: 0, w: h.x, h: this.height}), a({x: h.x, y: 0, w: this.width - h.x, h: h.y}), a({x: h.x + h.w, y: 0, w: this.width - h.x - h.w, h: this.height}), a({x: h.x, y: h.y + h.h, w: this.width - h.x, h: this.height - h.h - h.y})
    ...
    var h = a.dygraph, i = h.getOption("stepPlot", a.setName);
    Dygraph.isArrayLike(d) || (d = null);
    var j = h.getOption("drawGapEdgePoints", a.setName), k = a.points, l = Dygraph.createIterator(k, 0, k.length, DygraphCanvasRenderer._getIteratorPredicate(h.getOption("connectSeparatedPoints"))), m = d && d.length >= 2, n = a.drawingContext;
    ...
    for (var q = b.array_, r = b.end_, s = b.predicate_, t = b.start_; r > t; t++) {
        ...
        if (null === j.canvasy || j.canvasy != j.canvasy)g && null !== k && (p.moveTo(k, l), p.lineTo(j.canvasx, l)), k = l = null; else {
            if (i = !1, f || !k) {
    ...
    for (c = 0; c < i.length; c++)for (var l = i[c], m = c == i.length - 1, n = 0; n < f.length; n++)if (d = g[n], !a || d == a) {
        var o = f[n], p = l;
        if (d in j) {
            if (!m)continue;
            p = j[d]
        ...
        e.save(), e.strokeStyle = q, e.lineWidth = r, p({points: o, setName: d, drawingContext: e, color: q, strokeWidth: r, dygraph: this.dygraph_, axis: this.dygraph_.axisPropertiesForSeries(d), plotArea: this.area, seriesIndex: n, seriesCount: f.length, singleSeriesName: a, allSeriesPoints: f}), e.restore()
    }
    ...
    var b = a.dygraph, c = a.setName, d = a.strokeWidth, e = b.getOption("strokeBorderWidth", c), f = b.getOption("drawPointCallback", c) || Dygraph.Circles.DEFAULT, g = b.getOption("strokePattern", c), h = b.getOption("drawPoints", c), i = b.getOption("pointSize", c);
    ...
    var b = a.dygraph, c = a.setName, d = b.getOption("errorBars") || b.getOption("customBars");
    if (d) {
        var e = b.getOption("fillGraph", c);
        e && b.warn("Can't use fillGraph option with error bars");
        var f, g = a.drawingContext, h = a.color, i = b.getOption("fillAlpha", c), j = b.getOption("stepPlot", c), k = a.points, l = Dygraph.createIterator(k, 0, k.length, DygraphCanvasRenderer._getIteratorPredicate(b.getOption("connectSeparatedPoints"))), m = 0 / 0, n = 0 / 0, o = [-1, -1], p = new RGBColorParser(h), q = "rgba(" + p.r + "," + p.g + "," + p.b + "," + i + ")";
    ...
    if (!a.singleSeriesName && 0 === a.seriesIndex) {
        ...
        var e = function () {
            for (var a = 0; a < c.length; a++)if (b.getOption("fillGraph", c[a]))return!0;
            return!1
        }();
        if (e)for (var f, g, h = a.drawingContext, i = a.plotArea, j = a.allSeriesPoints, k = j.length, l = b.getOption("fillAlpha"), m = b.getOption("stackedGraph"), n = b.getColors(), o = {}, p = k - 1; p >= 0; p--) {
            var q = c[p];
            if (b.getOption("fillGraph", q)) {
                var r = b.getOption("stepPlot", q), s = n[p], t = b.axisPropertiesForSeries(q), u = 1 + t.minyval * t.yscale;
                0 > u ? u = 0 : u > 1 && (u = 1), u = i.h * u + i.y;
                var v, w = j[p], x = Dygraph.createIterator(w, 0, w.length, DygraphCanvasRenderer._getIteratorPredicate(b.getOption("connectSeparatedPoints"))), y = 0 / 0, z = [-1, -1], A = new RGBColorParser(s), B = "rgba(" + A.r + "," + A.g + "," + A.b + "," + l + ")";
...
Dygraph.NAME = "Dygraph", Dygraph.VERSION = "1.0.1", Dygraph.__repr__ = function () {
    ...
    var d, e = b("digitsAfterDecimal"), f = b("maxNumberWidth"), g = b("labelsKMB"), h = b("labelsKMG2");
    if (d = 0 !== a && (Math.abs(a) >= Math.pow(10, f) || Math.abs(a) < Math.pow(10, -e)) ? a.toExponential(e) : "" + Dygraph.round_(a, e), g || h) {
        ...
        for (var l = Math.abs(a), m = Dygraph.pow(i, j.length), n = j.length - 1; n >= 0; n--, m /= i)if (l >= m) {
    ...
    var b = Dygraph.zeropad, c = new Date(a), d = "" + c.getFullYear(), e = b(c.getMonth() + 1), f = b(c.getDate()), g = "", h = 3600 * c.getHours() + 60 * c.getMinutes() + c.getSeconds();
    ...
    for (var e = Dygraph.PLUGINS.concat(this.getOption("plugins")), f = 0; f < e.length; f++) {
        var g = e[f], h = new g, i = {plugin: h, events: {}, options: {}, pluginOptions: {}}, j = h.activate(this);
        for (var k in j)i.events[k] = j[k];
        this.plugins_.push(i)
    }
    for (var f = 0; f < this.plugins_.length; f++) {
        var l = this.plugins_[f];
        for (var k in l.events)if (l.events.hasOwnProperty(k)) {
            var m = l.events[k], n = [l.plugin, m];
    ...
    var c = {dygraph: this, cancelable: !1, defaultPrevented: !1, preventDefault: function () {
    ...
    if (null === a)return null;
    var c = this.plotter_.area, d = this.yAxisRange(b);
    if ("undefined" == typeof b && (b = 0), this.attributes_.getForAxis("logscale", b)) {
        var e = (a - c.y) / c.h, f = Dygraph.log10(d[1]), g = f - e * (f - Dygraph.log10(d[0])), h = Math.pow(Dygraph.LOG_SCALE, g);
        return h
    }
    ...
    var c, d = this.yAxisRange(b), e = this.attributes_.getForAxis("logscale", b);
    if (e) {
    ...
    this.mouseMoveHandler_ = function (a) {
        ...
        var c = a.target || a.fromElement, d = a.relatedTarget || a.toElement;
    ...
    var a = this.getLabels(), b = a.length - 1;
    this.colors_ = [], this.colorsMap_ = {};
    var c, d = this.attr_("colors");
    if (d) {
        for (c = 0; b > c; c++)if (this.visibility()[c]) {
            var e = d[c % d.length];
            this.colors_.push(e), this.colorsMap_[a[1 + c]] = e
        }
    } else {
        var f = this.attr_("colorSaturation") || 1, g = this.attr_("colorValue") || .5, h = Math.ceil(b / 2);
        for (c = 1; b >= c; c++)if (this.visibility()[c - 1]) {
            var i = c % 2 ? Math.ceil(c / 2) : h + c / 2, j = 1 * i / (1 + b), e = Dygraph.hsvToRGB(j, f, g);
    ...
    return-1 == b ? null : {name: a, column: b, visible: this.visibility()[b - 1], color: this.colorsMap_[a], axis: 1 + this.attributes_.axisForSeries(a)}
    ...
    var a = this.attr_("showRoller") ? "block" : "none", b = this.plotter_.area, c = {position: "absolute", zIndex: 10, top: b.y + b.h - 25 + "px", left: b.x + 1 + "px", display: a};
    ...
    var a = {isZooming: !1, isPanning: !1, is2DPan: !1, dragStartX: null, dragStartY: null, dragEndX: null, dragEndY: null, dragDirection: null, prevEndX: null, prevEndY: null, prevDragDirection: null, cancelNextDblclick: !1, initialLeftmostDate: null, xUnitsPerPixel: null, dateRange: null, px: 0, py: 0, boundedDates: null, boundedValues: null, tarp: new Dygraph.IFrameTarp, initializeMouseDown: function (a, b, c) {
    ...
    }}, b = this.attr_("interactionModel"), c = this, d = function (b) {
        return function (d) {
            b(d, c, a)
        }
    };
    ...
    var f, g, h = this.attr_("animatedZooms") ? Dygraph.ANIMATION_STEPS : 1, i = [], j = [];
    if (null !== a && null !== b)for (f = 1; h >= f; f++)g = Dygraph.zoomAnimationFunction(f, h), i[f - 1] = [a[0] * (1 - g) + g * b[0], a[1] * (1 - g) + g * b[1]];
    if (null !== c && null !== d)for (f = 1; h >= f; f++) {
    ...
    Dygraph.repeatAndCleanup(function (a) {
        if (j.length)for (var b = 0; b < m.axes_.length; b++) {
            var c = j[a][b];
    ...
    var b = Dygraph.pageX(a) - Dygraph.findPosX(this.mouseEventElement_), c = Dygraph.pageY(a) - Dygraph.findPosY(this.mouseEventElement_);
    return[b, c]
}, Dygraph.prototype.findClosestRow = function (a) {
    for (var b = 1 / 0, c = -1, d = this.layout_.points, e = 0; e < d.length; e++)for (var f = d[e], g = f.length, h = 0; g > h; h++) {
        var i = f[h];
    ...
    for (var c, d, e, f, g, h, i, j = 1 / 0, k = this.layout_.points.length - 1; k >= 0; --k)for (var l = this.layout_.points[k], m = 0; m < l.length; ++m)f = l[m], Dygraph.isValidPoint(f) && (d = f.canvasx - a, e = f.canvasy - b, c = d * d + e * e, j > c && (j = c, g = f, h = k, i = f.idx));
    ...
    for (var c, d, e = this.findClosestRow(a), f = 0; f < this.layout_.points.length; ++f) {
        var g = this.getLeftBoundary_(f), h = e - g, i = this.layout_.points[f];
        if (!(h >= i.length)) {
            var j = i[h];
    ...
    if (void 0 !== b && null !== b) {
        var c = this.eventToDomCoords(a), d = c[0], e = c[1], f = this.attr_("highlightSeriesOpts"), g = !1;
    ...
    var d = this.fadeLevel, e = 0 > a ? d : b - d;
    if (0 >= e)return void(this.fadeLevel && this.updateSelection_(1));
    var f = ++this.animateId, g = this;
    ...
    if (this.isUsingExcanvas_ && this.currentZoomRectArgs_ && Dygraph.prototype.drawZoomRect_.apply(this, this.currentZoomRectArgs_), this.selPoints_.length > 0) {
        ...
        for (c.save(), b = 0; b < this.selPoints_.length; b++) {
            ...
            if (Dygraph.isOK(k.canvasy)) {
                var l = this.attr_("highlightCircleSize", k.name), m = this.attr_("drawHighlightPointCallback", k.name), n = this.plotter_.colors[k.name];
    ...
    var b = this.optionsViewForAxis_("x"), c = b("ticker")(a[0], a[1], this.width_, b, this);
    this.layout_.setXTicks(c)
}, Dygraph.prototype.extremeValues_ = function (a) {
    var b, c, d = null, e = null, f = this.attr_("errorBars") || this.attr_("customBars");
    if (f) {
        for (b = 0; b < a.length; b++)if (c = a[b][1][0], null !== c && !isNaN(c)) {
            var g = c - a[b][1][1], h = c + a[b][1][2];
            g > c && (g = c), c > h && (h = c), (null === e || h > e) && (e = h), (null === d || d > g) && (d = g)
        }
    } else for (b = 0; b < a.length; b++)c = a[b][1], null === c || isNaN(c) || ((null === e || c > e) && (e = c), (null === d || d > c) && (d = c));
    return[d, e]
    ...
    for (var b = 1; b < this.numColumns(); b++) {
        var c = this.attr_("logscale"), d = this.extractSeries_(this.rawData_, b, c);
    ...
    for (var e = [], f = 0; f < a.length; ++f) {
        var g = a[f], h = b ? g[1][0] : g[1], i = null === h ? null : DygraphLayout.parseFloat_(h), j = {x: 0 / 0, y: 0 / 0, xval: DygraphLayout.parseFloat_(g[0]), yval: i, name: c, idx: f + d};
    ...
    var c, d, e, f = [], g = [], h = [], i = {}, j = this.attr_("errorBars"), k = this.attr_("customBars"), l = j || k, m = function (a) {
        return l ? k ? null === a[1][1] : j ? null === a[1][0] : !1 : null === a[1]
    }, n = a.length - 1;
    for (c = n; c >= 1; c--)if (this.visibility()[c - 1]) {
        if (b) {
            e = a[c];
            var o = b[0], p = b[1], q = null, r = null;
        ...
        var v = this.attr_("labels")[c], w = this.extremeValues_(e), x = Dygraph.seriesToPoints_(e, l, v, f[c - 1][0]);
    ...
    if (this.cascadeEvents_("willDrawChart", b), this.plotter_.render(), this.cascadeEvents_("didDrawChart", b), this.lastRow_ = -1, this.canvas_.getContext("2d").clearRect(0, 0, this.canvas_.width, this.canvas_.height), null !== this.attr_("drawCallback") && this.attr_("drawCallback")(this, a), a)for (this.readyFired_ = !0; this.readyFns_.length > 0;) {
        var c = this.readyFns_.pop();
        c(this)
    }
}, Dygraph.prototype.computeYAxes_ = function () {
    var a, b, c, d, e;
    if (void 0 !== this.axes_ && this.user_attrs_.hasOwnProperty("valueRange") === !1)for (a = [], c = 0; c < this.axes_.length; c++)a.push(this.axes_[c].valueWindow);
    for (this.axes_ = [], b = 0; b < this.attributes_.numAxes(); b++)d = {g: this}, Dygraph.update(d, this.attributes_.axisOptions(b)), this.axes_[b] = d;
    ...
    for (b = 0; b < this.axes_.length; b++)if (0 === b)d = this.optionsViewForAxis_("y" + (b ? "2" : "")), e = d("valueRange"), e && (this.axes_[b].valueRange = e); else {
    ...
    for (var b, c, d, e, f, g = function (a) {
        ...
        if (d = this.attributes_.seriesForAxis(i), b = !0, e = .1, null !== this.attr_("yRangePad") && (b = !1, e = this.attr_("yRangePad") / this.plotter_.area.h), 0 === d.length)j.extremeRange = [0, 1]; else {
    ...
    for (var d = [], e = this.attr_("errorBars"), f = this.attr_("customBars"), g = 0; g < a.length; g++) {
        var h = a[g][0], i = a[g][b];
        if (c)if (e || f) {
            for (var j = 0; j < i.length; j++)if (i[j] <= 0) {
                i = null;
                break
            }
        } else 0 >= i && (i = null);
    ...
    var c, d, e, f, g, h, i, j, k = [], l = this.attr_("sigma");
    if (this.fractions_) {
        var m = 0, n = 0, o = 100;
        for (e = 0; e < a.length; e++) {
            m += a[e][1][0], n += a[e][1][1], e - b >= 0 && (m -= a[e - b][1][0], n -= a[e - b][1][1]);
            var p = a[e][0], q = n ? m / n : 0;
            if (this.attr_("errorBars"))if (this.attr_("wilsonInterval"))if (n) {
                var r = 0 > q ? 0 : q, s = n, t = l * Math.sqrt(r * (1 - r) / s + l * l / (4 * s * s)), u = 1 + l * l / n;
                c = (r + l * l / (2 * n) - t) / u, d = (r + l * l / (2 * n) + t) / u, k[e] = [p, [r * o, (r - c) * o, (d - r) * o]]
            } else k[e] = [p, [0, 0, 0]]; else j = n ? l * Math.sqrt(q * (1 - q) / n) : 1, k[e] = [p, [o * q, o * j, o * j]]; else k[e] = [p, o * q]
        }
        ...
        for (e = 0; e < a.length; e++) {
            var x = a[e][1];
            if (g = x[1], k[e] = [a[e][0], [g, g - x[0], x[2] - g]], null === g || isNaN(g) || (c += x[0], v += g, d += x[2], w += 1), e - b >= 0) {
                var y = a[e - b];
                null === y[1][1] || isNaN(y[1][1]) || (c -= y[1][0], v -= y[1][1], d -= y[1][2], w -= 1)
            }
            k[e] = w ? [a[e][0], [1 * v / w, 1 * (v - c) / w, 1 * (d - v) / w]] : [a[e][0], [null, null, null]]
        }
    } else if (this.attr_("errorBars"))for (e = 0; e < a.length; e++) {
        h = 0;
        var z = 0;
        for (i = 0, f = Math.max(0, e - b + 1); e + 1 > f; f++)g = a[f][1][0], null === g || isNaN(g) || (i++, h += a[f][1][0], z += Math.pow(a[f][1][1], 2));
        if (i)j = Math.sqrt(z) / i, k[e] = [a[e][0], [h / i, l * j, l * j]]; else {
    ...
    var b, c, d = [], e = Dygraph.detectLineDelimiter(a), f = a.split(e || "\n"), g = this.attr_("delimiter");
    ...
    for (var i, j = 0, k = !1, l = this.attr_("labels").length, m = !1, n = h; n < f.length; n++) {
        ...
        if (j = n, 0 !== o.length && "#" != o[0]) {
            ...
            if (!(p.length < 2)) {
                ...
                } else for (c = 1; c < p.length; c++)q[c] = this.parseFloat_(p[c], n, o);
    ...
    var b = function (a) {
        ...
        return b
    }, c = a.getNumberOfColumns(), d = a.getNumberOfRows(), e = a.getColumnType(0);
    if ("date" == e || "datetime" == e)this.attrs_.xValueParser = Dygraph.dateParser, this.attrs_.axes.x.valueFormatter = Dygraph.dateString_, this.attrs_.axes.x.ticker = Dygraph.dateTicker, this.attrs_.axes.x.axisLabelFormatter = Dygraph.dateAxisFormatter; else {
    ...
    for (f = 1; c > f; f++) {
        ...
        if ("number" == k)h.push(f); else if ("string" == k && this.attr_("displayAnnotations")) {
            var l = h[h.length - 1];
    ...
    for (f = 0; d > f; f++) {
        ...
        if ("undefined" != typeof a.getValue(f, 0) && null !== a.getValue(f, 0)) {
            if (q.push("date" == e || "datetime" == e ? a.getValue(f, 0).getTime() : a.getValue(f, 0)), this.attr_("errorBars"))for (g = 0; c - 1 > g; g++)q.push([a.getValue(f, 1 + 2 * g), a.getValue(f, 2 + 2 * g)]); else {
    ...
    var a = this.file_;
    if ("function" == typeof a && (a = a()), Dygraph.isArrayLike(a))this.rawData_ = this.parseArray_(a), this.predraw_(); else if ("object" == typeof a && "function" == typeof a.getColumnRange)this.parseDataTable_(a), this.predraw_(); else if ("string" == typeof a) {
        var b = Dygraph.detectLineDelimiter(a);
        if (b)this.loadedEvent_(a); else {
            var c;
    ...
    var c = a.file, d = Dygraph.mapLegacyOptions_(a);
    ...
    if (!Dygraph.addedAnnotationCSS) {
        var a = "border: 1px solid black; background-color: white; text-align: center;", b = document.createElement("style");
        b.type = "text/css", document.getElementsByTagName("head")[0].appendChild(b);
        for (var c = 0; c < document.styleSheets.length; c++)if (!document.styleSheets[c].disabled) {
...
Dygraph.LOG_SCALE = 10, Dygraph.LN_TEN = Math.log(Dygraph.LOG_SCALE), Dygraph.log10 = function (a) {
    ...
    if ("undefined" != typeof printStackTrace)try {
        for (c = printStackTrace({guess: !1}); -1 != c[0].indexOf("stacktrace");)c.splice(0, 1);
        c.splice(0, 2);
    ...
    if ("undefined" != typeof window.console) {
        var g = window.console, h = function (a, b, c) {
    ...
    Dygraph.addEvent(a, b, c), this.registeredEvents_.push({elem: a, type: b, fn: c})
    ...
    var b = Dygraph.zeropad, c = new Date(a);
    ...
    if ("undefined" != typeof b && null !== b)for (var d in b)b.hasOwnProperty(d) && (null === b[d] ? a[d] = null : Dygraph.isArrayLike(b[d]) ? a[d] = b[d].slice() : c(b[d]) ? a[d] = b[d] : "object" == typeof b[d] ? (("object" != typeof a[d] || null === a[d]) && (a[d] = {}), Dygraph.updateDeep(a[d], b[d])) : a[d] = b[d]);
    return a
    ...
    var h = f, i = h, j = function () {
        var a = d + Math.sin(i) * c, b = e + -Math.cos(i) * c;
        return[a, b]
    }, k = j(), l = k[0], m = k[1];
    a.moveTo(l, m);
    for (var n = 0; b > n; n++) {
...
}, Dygraph.Circles = {DEFAULT: function (a, b, c, d, e, f, g) {
    c.beginPath(), c.fillStyle = f, c.arc(d, e, g, 0, 2 * Math.PI, !1), c.fill()
}, TRIANGLE: Dygraph.shapeFunction_(3), SQUARE: Dygraph.shapeFunction_(4, Math.PI / 4), DIAMOND: Dygraph.shapeFunction_(4), PENTAGON: Dygraph.shapeFunction_(5), HEXAGON: Dygraph.shapeFunction_(6), CIRCLE: function (a, b, c, d, e, f, g) {
    c.beginPath(), c.strokeStyle = f, c.fillStyle = "white", c.arc(d, e, g, 0, 2 * Math.PI, !1), c.fill(), c.stroke()
}, STAR: Dygraph.shapeFunction_(5, 0, 4 * Math.PI / 5), PLUS: function (a, b, c, d, e, f, g) {
    ...
    for (var a = document.getElementsByTagName("iframe"), b = 0; b < a.length; b++) {
        var c = a[b], d = Dygraph.findPosX(c), e = Dygraph.findPosY(c), f = c.offsetWidth, g = c.offsetHeight, h = document.createElement("div");
...
}, Dygraph.dateSetters = {ms: Date.prototype.setMilliseconds, s: Date.prototype.setSeconds, m: Date.prototype.setMinutes, h: Date.prototype.setHours}, Dygraph.setDateSameTZ = function (a, b) {
    ...
    if (0 > b)return a;
    for (var c = this.date_graph.layout_.points, d = 0; d < c.length; ++d)a.push({row: b, column: d + 1});
    return a
    ...
    if (c.dateRange = f[1] - f[0], c.initialLeftmostDate = f[0], c.xUnitsPerPixel = c.dateRange / (b.plotter_.area.w - 1), b.attr_("panEdgeFraction")) {
        var g = b.width_ * b.attr_("panEdgeFraction"), h = b.xAxisExtremes(), i = b.toDomXCoord(h[0]) - g, j = b.toDomXCoord(h[1]) + g, k = b.toDataXCoord(i), l = b.toDataXCoord(j);
        c.boundedDates = [k, l];
        var m = [], n = b.height_ * b.attr_("panEdgeFraction");
    ...
    var e = d + c.dateRange;
    if (c.boundedDates && e > c.boundedDates[1] && (d -= e - c.boundedDates[1], e = d + c.dateRange), b.dateWindow_ = [d, e], c.is2DPan)for (var f = c.dragEndY - c.dragStartY, g = 0; g < b.axes_.length; g++) {
    ...
    var d = a.attr_("clickCallback"), e = a.attr_("pointClickCallback"), f = null;
    if (e) {
    ...
    if (d >= 10 && c.dragDirection == Dygraph.HORIZONTAL) {
        var g = Math.min(c.dragStartX, c.dragEndX), h = Math.max(c.dragStartX, c.dragEndX);
        ...
        var i = Math.min(c.dragStartY, c.dragEndY), j = Math.max(c.dragStartY, c.dragEndY);
    ...
    for (var d = [], e = 0; e < a.touches.length; e++) {
        var f = a.touches[e];
        d.push({pageX: f.pageX, pageY: f.pageY, dataX: b.toDataXCoord(f.pageX), dataY: b.toDataYCoord(f.pageY)})
    }
    if (c.initialTouches = d, 1 == d.length)c.initialPinchCenter = d[0], c.touchDirections = {x: !0, y: !0}; else if (d.length >= 2) {
        c.initialPinchCenter = {pageX: .5 * (d[0].pageX + d[1].pageX), pageY: .5 * (d[0].pageY + d[1].pageY), dataX: .5 * (d[0].dataX + d[1].dataX), dataY: .5 * (d[0].dataY + d[1].dataY)};
        var g = 180 / Math.PI * Math.atan2(c.initialPinchCenter.pageY - d[0].pageY, d[0].pageX - c.initialPinchCenter.pageX);
        g = Math.abs(g), g > 90 && (g = 90 - g), c.touchDirections = {x: 67.5 > g, y: g > 22.5}
    }
    ...
    g = 1 == e.length ? e[0] : {pageX: .5 * (e[0].pageX + e[1].pageX), pageY: .5 * (e[0].pageY + e[1].pageY)};
    var j = {pageX: g.pageX - i.pageX, pageY: g.pageY - i.pageY}, k = c.initialRange.x[1] - c.initialRange.x[0], l = c.initialRange.y[0] - c.initialRange.y[1];
    ...
    var q = !1;
    if (c.touchDirections.x && (b.dateWindow_ = [i.dataX - j.dataX + (c.initialRange.x[0] - i.dataX) / m, i.dataX - j.dataX + (c.initialRange.x[1] - i.dataX) / m], q = !0), c.touchDirections.y)for (d = 0; 1 > d; d++) {
...
}, Dygraph.Interaction.defaultModel = {mousedown: function (a, b, c) {
...
}}, Dygraph.DEFAULT_ATTRS.interactionModel = Dygraph.Interaction.defaultModel, Dygraph.defaultInteractionModel = Dygraph.Interaction.defaultModel, Dygraph.endZoom = Dygraph.Interaction.endZoom, Dygraph.moveZoom = Dygraph.Interaction.moveZoom, Dygraph.startZoom = Dygraph.Interaction.startZoom, Dygraph.endPan = Dygraph.Interaction.endPan, Dygraph.movePan = Dygraph.Interaction.movePan, Dygraph.startPan = Dygraph.Interaction.startPan, Dygraph.Interaction.nonInteractiveModel_ = {mousedown: function (a, b, c) {
    c.initializeMouseDown(a, b, c)
}, mouseup: function (a, b, c) {
    c.dragEndX = b.dragGetX_(a, c), c.dragEndY = b.dragGetY_(a, c);
    var d = Math.abs(c.dragEndX - c.dragStartX), e = Math.abs(c.dragEndY - c.dragStartY);
    2 > d && 2 > e && void 0 !== b.lastx_ && -1 != b.lastx_ && Dygraph.Interaction.treatMouseOpAsClick(b, a, c)
}}, Dygraph.Interaction.dragIsPanInteractionModel = {mousedown: function (a, b, c) {
    ...
    if (f)for (g = 0; g < f.length; g++)l.push({v: f[g]}); else {
        if (d("logscale")) {
            j = Math.floor(c / k);
            ...
            if (n - m >= j / 4) {
                for (var p = n; p >= m; p--) {
                    var q = Dygraph.PREFERRED_LOG_TICK_VALUES[p], r = Math.log(q / a) / Math.log(b / a) * c, s = {v: q};
                    null === o ? o = {tickValue: q, pixel_coord: r} : Math.abs(r - o.pixel_coord) >= k ? o = {tickValue: q, pixel_coord: r} : s.label = "", l.push(s)
                }
                l.reverse()
            }
        }
        if (0 === l.length) {
            ...
            for (x > y && (w *= -1), g = 0; j > g; g++)i = x + g * w, l.push({v: i})
        }
    }
    ...
    var e = Dygraph.LONG_TICK_PLACEMENTS[c], f = 31557807360, g = 1 * (b - a) / f;
    return Math.floor(.5 + 1 * g * e.months.length / e.year_mod)
},Dygraph.getDateAxis = function (a, b, c, d, e) {
    var f, g = d("axisLabelFormatter"), h = [];
    if (c < Dygraph.MONTHLY) {
        var i = Dygraph.SHORT_SPACINGS[c], j = i / 1e3, k = new Date(a);
        Dygraph.setDateSameTZ(k, {ms: 0});
        var l;
        60 >= j ? (l = k.getSeconds(), Dygraph.setDateSameTZ(k, {s: l - l % j})) : (Dygraph.setDateSameTZ(k, {s: 0}), j /= 60, 60 >= j ? (l = k.getMinutes(), Dygraph.setDateSameTZ(k, {m: l - l % j})) : (Dygraph.setDateSameTZ(k, {m: 0}), j /= 60, 24 >= j ? (l = k.getHours(), k.setHours(l - l % j)) : (k.setHours(0), j /= 24, 7 == j && k.setDate(k.getDate() - k.getDay())))), a = k.getTime();
        var m = new Date(a).getTimezoneOffset(), n = i >= Dygraph.SHORT_SPACINGS[Dygraph.TWO_HOURLY];
        for (f = a; b >= f; f += i) {
        ...
        for (var r = new Date(a).getFullYear(), s = new Date(b).getFullYear(), t = Dygraph.zeropad, u = r; s >= u; u++)if (u % q === 0)for (var v = 0; v < p.length; v++) {
            var w = u + "/" + t(1 + p[v]) + "/01";
            f = Dygraph.dateStrToMillis(w), a > f || f > b || h.push({v: f, label: g(new Date(f), c, d, e)})
        }
    }
    return h
    ...
    return a.prototype.toString = function () {
        ...
        if (c && 0 !== c.length)for (var d = a.canvas.parentNode, e = {position: "absolute", fontSize: b.getOption("axisLabelFontSize") + "px", zIndex: 10, overflow: "hidden"}, f = function (a, c, d) {
            return function (e) {
                var f = d.annotation;
    ...
    return a.prototype.toString = function () {
        ...
        if (d.getOption("drawXAxis") || d.getOption("drawYAxis")) {
            var e, f, g, h, i, j = a.drawingContext, k = a.canvas.parentNode, l = a.canvas.width, m = a.canvas.height, n = function (a) {
                return{position: "absolute", fontSize: d.getOptionForAxis("axisLabelFontSize", a) + "px", zIndex: 10, color: d.getOptionForAxis("axisLabelColor", a), width: d.getOption("axisLabelWidth") + "px", lineHeight: "normal", overflow: "hidden"}
            }, o = {x: n("x"), y: n("y"), y2: n("y2")}, p = function (a, b, c) {
            ...
            j.save();
            var q = d.layout_, r = a.dygraph.plotter_.area;
            if (d.getOption("drawYAxis")) {
                if (q.yticks && q.yticks.length > 0) {
                    ...
                    var x = this.ylabels_[0], v = d.getOptionForAxis("axisLabelFontSize", "y"), y = parseInt(x.style.top, 10) + v;
            ...
            if (d.getOption("drawXAxis")) {
                if (q.xticks)for (i = 0; i < q.xticks.length; i++) {
    ...
    a.prototype.activate = function (a) {
        ...
        if (d && null !== d)b = "string" == typeof d || d instanceof String ? document.getElementById(d) : d; else {
            var e = {position: "absolute", fontSize: "14px", zIndex: 10, width: c + "px", top: "0px", left: a.size().width - c - 2 + "px", background: "white", lineHeight: "normal", textAlign: "left", overflow: "hidden"};
        ...
        return this.legend_div_ = b, this.one_em_width_ = 10, {select: this.select, deselect: this.deselect, predraw: this.predraw, didDrawChart: this.didDrawChart}
    };
    var d = function (a) {
    ...
    return a.prototype.select = function (a) {
        var c = a.selectedX, d = a.selectedPoints, e = b(a.dygraph, c, d, this.one_em_width_);
        ...
        var m = a.optionsViewForAxis_("x"), n = m("valueFormatter");
        f = n(b, m, k[0], a), "" !== f && (f += ":");
        var o = [], p = a.numAxes();
        ...
        for (h = 0; h < d.length; h++) {
            ...
            if ((0 !== s.yval || q) && Dygraph.isOK(s.canvasy)) {
                g && (f += "<br/>");
                var l = a.getPropertiesForSeries(s.name), t = o[l.axis - 1], u = t("valueFormatter"), v = u(s.yval, t, s.name, a), w = s.name == r ? " class='highlight'" : "";
        ...
        var m = "";
        for (f = 0; i > f; f++)for (e = 0; k > e; e += 2)g = l[e % l.length], h = e < a.length ? l[(e + 1) % l.length] : 0, m += '<div style="display: inline-block; position: relative; bottom: .5ex; margin-right: ' + h + "em; padding-left: " + g + "em; height: 1px; border-bottom: 2px solid " + b + ';"></div>';
        return m
    }, a
    ...
    return a.prototype.toString = function () {
        ...
        return this.dygraph_ = a, this.isUsingExcanvas_ = a.isUsingExcanvas_, this.getOption_("showRangeSelector") && this.createInterface_(), {layout: this.reserveSpace_, predraw: this.renderStaticLayer_, didDrawChart: this.renderInteractiveLayer_}
    }, a.prototype.destroy = function () {
        ...
        this.createCanvases_(), this.isUsingExcanvas_ && this.createIEPanOverlay_(), this.createZoomHandles_(), this.initInteraction_(), this.getOption_("animatedZooms") && (this.dygraph_.warn("Animated zooms and range selector are not compatible; disabling animatedZooms."), this.dygraph_.updateOptions({animatedZooms: !1}, !0)), this.interfaceCreated_ = !0, this.addToGraph_()
        ...
        this.getOption_("drawXAxis") && (c = this.getOption_("xAxisHeight") || this.getOption_("axisLabelFontSize") + 2 * this.getOption_("axisTickSize")), this.canvasRect_ = {x: b.x, y: b.y + b.h + c + 4, w: b.w, h: this.getOption_("rangeSelectorHeight")}, a(this.bgcanvas_, this.canvasRect_), a(this.fgcanvas_, this.canvasRect_)
        ...
        var a, b, c, d, e, f, g, h, i, j, k, l, m, n, o = this, p = this.isIE_ ? document : window, q = 0, r = null, s = !1, t = !1, u = !this.isMobileDevice_ && !this.isUsingExcanvas_, v = new Dygraph.IFrameTarp;
        a = function (a) {
        ...
        var a, b, c, d, e, f, g, h, i = this.dygraph_.rawData_, j = this.getOption_("logscale"), k = [];
        for (d = 0; d < i.length; d++)if (i[d].length > 1 && null !== i[d][1]) {
            if (c = "number" != typeof i[d][1])for (a = [], b = [], f = 0; f < i[d][1].length; f++)a.push(0), b.push(0);
            break
        }
        for (d = 0; d < i.length; d++) {
            var l = i[d];
            if (g = l[0], c)for (f = 0; f < a.length; f++)a[f] = b[f] = 0; else a = b = 0;
            for (e = 1; e < l.length; e++)if (this.dygraph_.visibility()[e - 1]) {
                var m;
                if (c)for (f = 0; f < a.length; f++)m = l[e][f], null === m || isNaN(m) || (a[f] += m, b[f]++); else {
        ...
        if (k = this.dygraph_.rollingAverage(k, this.dygraph_.rollPeriod_), "number" != typeof k[0][1])for (d = 0; d < k.length; d++)h = k[d][1], k[d][1] = h[0];
        var n = Number.MAX_VALUE, o = -Number.MAX_VALUE;
        for (d = 0; d < k.length; d++)h = k[d][1], null !== h && isFinite(h) && (!j || h > 0) && (n = Math.min(n, h), o = Math.max(o, h));
        var p = .25;
        if (j)for (o = Dygraph.log10(o), o += o * p, n = Dygraph.log10(n), d = 0; d < k.length; d++)k[d][1] = Dygraph.log10(k[d][1]); else {
            var q, r = o - n;
