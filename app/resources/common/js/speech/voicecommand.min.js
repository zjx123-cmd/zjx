...
var root = this, SpeechRecognition = root.SpeechRecognition || root.webkitSpeechRecognition || root.mozSpeechRecognition || root.msSpeechRecognition || root.oSpeechRecognition;
...
(function (a) {
    ...
    var b, c, d = [], e = {start: [], error: [], end: [], result: [], resultMatch: [], resultNoMatch: [], errorNetwork: [], errorPermissionBlocked: [], errorPermissionDenied: []}, f = 0, g = !1, h = "font-weight: bold; color: #00f;", i = /\s*\((.*?)\)\s*/g, j = /(\(\?:[^)]+\))\?/g, k = /(\(\?)?:\w+/g, l = /\*\w+/g, m = /[\-{}\[\]+?.,\\\^$|#]/g, n = function (a) {
    ...
    root.smartSpeechRecognition = {init: function (i, j) {
        j = j === a ? !0 : !!j, b && b.abort && b.abort(), b = new SpeechRecognition, b.maxAlternatives = 5, b.continuous = !0, b.lang = voice_command_lang || "en-US", b.onstart = function () {
            ...
            switch (o(e.error), a.error) {
                ...
                case"service-not-allowed":
                    c = !1, $.root_.removeClass("service-allowed"), $.root_.addClass("service-not-allowed"), o((new Date).getTime() - f < 200 ? e.errorPermissionBlocked : e.errorPermissionDenied)
            }
        }, b.onend = function () {
            if (o(e.end), c) {
            ...
            for (var b, c = a.results[a.resultIndex], f = 0; f < c.length; f++) {
                b = c[f].transcript.trim(), g && root.console.log("Speech recognized: %c" + b, h);
                for (var i = 0, j = d.length; j > i; i++) {
                    var k = d[i].command.exec(b);
                    if (k) {
                        var l = k.slice(1);
                        g && (root.console.log("command matched: %c" + d[i].originalPhrase, h), l.length && root.console.log("with parameters", l)), d[i].callback.apply(this, l), o(e.resultMatch);
                        var m = ["sound on", "mute", "stop"];
                        return m.indexOf(d[i].originalPhrase) < 0 && ($.smallBox({title: d[i].originalPhrase, content: "loading...", color: "#333", sound_file: "voice_alert", timeout: 2e3}), $("#speech-btn .popover").is(":visible") && $("#speech-btn .popover").fadeOut(250)), !0
                    }
                }
            }
            return o(e.resultNoMatch), $.smallBox({title: 'Error: <strong> " ' + b + ' " </strong> no match found!', content: "Please speak clearly into the microphone", color: "#a90329", timeout: 5e3, icon: "fa fa-microphone"}), $("#speech-btn .popover").is(":visible") && $("#speech-btn .popover").fadeOut(250), !1
        }, j && (d = []), i.length && this.addCommands(i)
    }, start: function (d) {
